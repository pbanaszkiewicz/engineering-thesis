...
\section{Wymagania wydajnościowe}
\label{sec:wymagania_wydajnosciowe}

Na podstawie opisu z rozdziału \ref{sec:cele_projektu} wyznaczono poniższe cele wydajnościowe systemu ESAV:
...
\item System ESAV musi pracować poprawnie na dystansie 30 metrów w otwartej przestrzeni.
...
\begin{enumerate}
\item Moduł B systemu ESAV nie może razić prądem.
\item Komunikacja pomiędzy modułami powinna zabezpieczać przed atakami powtórzenia poleceń operatora.
\item Moduł B systemu ESAV nie może nagrzewać się powyżej temperatury $100\,^{\circ}\mathrm{C}$.
\end{enumerate}
